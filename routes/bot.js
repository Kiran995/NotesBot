...
//     mongodb.MongoClient.connect('mongodb://localhost:3000', function (err, db) {
//         if (err) {
//             throw err;
//         }
...
//     getFiles: function(courseCode, callback) {
//       kunotes_db.model("files").findOne({courseCode:courseCode, {type:1},
//            function(err, data) {
//              if (err) {
//                myText = "Error";
//              }
...
(function () {
  "use strict";
  var collection = kunotes_db.collection();
  ...
  function Bot (name) {
    ...
    this.actions = {
      changeName: function (newName) {
        ...
        //var title = Database.getFiles(courseCode, function(theText) { alert(theText); });

        this.sendBotMessage(greet[Math.floor(Math.random() * greet.length)] + ', ' + this.userName + '!', null);
      //   if(title == 1){  
      //   this.sendBotMessage('Found', null);
      // }
      // else{
      //   this.sendBotMessage('Could not find what you are looking. Come back again later', null);
      // }
      },
 ...
 //      },
  ...
  if(window.open('https://www.google.com/search?q=' + encodeURI(query))) {
        ...
        // database: {
        // db: function (query) {
        //   Database.getFiles(courseCode, function(theText) { alert(theText); });
    ...
    this.reactsTo = [
      ...
      //{pattern: /search (?:(?:on )?(?:database ))?for (.+)/i, reaction: {action: this.actions.search.database}, description: 'me to search on Google', confirm: false, special: false},
      {pattern: /search (?:(?:on )?(?:Database ))?for (.+)/i, reaction: {action: this.actions.search.database}, description: 'me to search on Database', confirm: false, special: false},
